...
Terminal=false
Type=Application
Categories=SteamLibrary; # Ajout important pour les jeux non steam
EOF
}
...
update-game-entries() {
    ...
    for library in $(steam-libraries); do
        ...
        for manifest in "$library"/steamapps/appmanifest_*.acf; do
            ...
            boxart=$(find $STEAM_ROOT/appcache/librarycache/${appid} -type f -name 'library_600x900.jpg' -print -quit)
