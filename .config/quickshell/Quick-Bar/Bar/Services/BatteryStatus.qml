...
Item {
    ...
    readonly property color iconColor: {
        if (full)      return Appearance.active
        if (charging)  return Appearance.active
    ...
    Row {
        ...
        Item {
            ...
            clip: true
            anchors.verticalCenter: parent.verticalCenter
            Behavior on implicitWidth { NumberAnimation { duration: Appearance.animDuration; easing.type: Easing.InOutCubic } }

            Text {
                id: pctText
                text:           root.pct + "%"
                color:          hov.hovered ? Appearance.active : Appearance.text
                font.pixelSize: 12
                anchors.verticalCenter: parent.verticalCenter
