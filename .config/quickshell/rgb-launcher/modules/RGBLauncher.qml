...
Rectangle {
    ...
    Process {
        id: modesProcess
        command: ["python3", "./service/backend.py"]
        running: false
    ...
    Process {
        ...
        command: [
            "cat",
            Qt.resolvedUrl("./script/conf/sequence.txt")
                .toString()
                .replace("file://", "")
        ]
        running: false
    ...
    function saveBrightness(brightness) {
        ...
        const cmd = "echo " + brightness + " > $HOME/.config/quickshell/rgb-launcher/modules/script/conf/brightness.txt";
