...
Singleton {
    ...
    property QtObject m3colors: {
        if (Common.Config.options.appearance.useMatugenColors && matugenLoader.item)
            return matugenLoader.item
        if (Common.Config.options.appearance.useWallustColors && wallustLoader.item)
            return wallustLoader.item
    ...
    Loader {
        id: wallustLoader
        active: Common.Config.options.appearance.useWallustColors
        source: "Appearance.wallust.qml"
        onStatusChanged: console.log("[Wallust] loader status:", status, "item:", item)
        onLoaded: console.log("[Wallust] loaded, m3primary:", item.m3primary)
    }
